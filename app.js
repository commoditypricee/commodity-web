...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // İlk açılışta robotun güncellediği data.json'ı çeker
    fetchMarketData(true).then(() => {
        // Ziyaretçi sayfada kaldığı sürece ekranı canlı tutan simülatör başlar
        startLiveTicker();
        
        // Ziyaretçi sayfayı kapatmasa bile her 5 dakikada bir arkaplanda güncellenen data.json'ı tekrar okur
        setInterval(() => fetchMarketData(false), 300000);
    });
...
// Ziyaretçinin API'sini tüketmeyen, tamamen güvenli canlı akış illüzyonu
...
// (Diğer renderGoldPriceTable, updateCardPercentages, applyTimeFilter ve loadCustomApexChart fonksiyonları önceki kodun BİREBİR aynısıdır. Dosya çok uzamasın diye kesmedim, alt kısmın tamamı aynı kalacak)

function renderGoldPriceTable(item) {
    const container = document.getElementById('perf-stats');
    if (!container) return;
    const periods = [{ label: 'Today', days: 1 }, { label: '30 Days', days: 30 }, { label: '6 Months', days: 180 }, { label: '1 Year', days: 365 }, { label: '5 Years', days: 1825 }];
    let tbodyHtml = '';
    const history = item.history || [];

    periods.forEach(p => {
        let changePercent = 0; let changeAmount = 0;
        ...
        if (sourceData && sourceData.length > 0) {
            ...
            changeAmount = currentPrice - startPrice; changePercent = (changeAmount / startPrice) * 100;
        } else if (p.days === 1) {
            changePercent = parseFloat(item.changePercent) || 0; changeAmount = item.price - (item.price / (1 + (changePercent/100)));
        }
        if (p.days === 1) {
            changePercent = parseFloat(item.changePercent); changeAmount = item.price - (item.price / (1 + (changePercent/100)));
        }
        const isPos = changePercent >= 0; const colorClass = isPos ? 'gp-pos' : 'gp-neg'; const sign = isPos ? '+' : '';
        tbodyHtml += `<tr><td class="left">${p.label}</td><td class="right ${colorClass}">${sign}${changeAmount.toFixed(2)}</td><td class="right ${colorClass}">${sign}${changePercent.toFixed(2)}%</td></tr>`;
    });
    container.innerHTML = `<div class="gp-title">${item.name.toUpperCase()} PERFORMANCE (USD)</div><table class="gp-simple-table"><thead><tr><th class="left">Period</th><th class="right">Amount</th><th class="right">%</th></tr></thead><tbody>${tbodyHtml}</tbody></table>`;
}
...
function interpolateWithVolatility(data) {
    ...
    for (let i = 0; i < data.length - 1; i++) {
        let p1 = data[i]; let p2 = data[i+1]; result.push(p1);
        let timeDiff = p2.x - p1.x; let interval = 10 * 60 * 1000; let steps = Math.floor(timeDiff / interval);
        if (steps > 1) {
            let timeStep = timeDiff / steps; let priceStep = (p2.y - p1.y) / steps;
            for (let j = 1; j < steps; j++) {
                let basePrice = p1.y + (priceStep * j); let volatilityNoise = (Math.random() - 0.5) * (basePrice * 0.0006); 
...
function applyTimeFilter(days, isLiveUpdate = false) {
    ...
    let filteredData = []; let isIntraday = (days === 1);
    if (isIntraday) {
    ...
    const prices = filteredData.map(h => h.y); const minPrice = Math.min(...prices); const maxPrice = Math.max(...prices);

    if (isLiveUpdate) { mainApexChart.updateSeries([{ name: 'Price', data: filteredData }], false); } 
    else { mainApexChart.updateSeries([{ name: 'Price', data: filteredData }]); }
    ...
    mainApexChart.updateOptions({
        xaxis: {
            ...
            labels: {
                ...
                formatter: function(val) {
                    if (!val) return ''; const date = new Date(val);
        ...
        yaxis: { min: minPrice - (minPrice * 0.002), max: maxPrice + (maxPrice * 0.002), labels: { style: { colors: '#334155', fontSize: '13px', fontFamily: 'Inter', fontWeight: 600 }, formatter: (value) => `$${value.toFixed(2)}` } },
        tooltip: {
            x: { formatter: function(val) {
                const date = new Date(val); const dayMonthYear = date.toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' });
...
function loadCustomApexChart(item) {
    ...
    currentItemHistory = item.history; currentItemIntraday = item.intraday || []; currentSymbolName = item.name;
    const activeBtn = document.querySelector('.time-btn.active'); const days = activeBtn ? parseInt(activeBtn.getAttribute('data-days')) : 1; const isIntraday = (days === 1);
    ...
    } else { filteredData = currentItemHistory.filter(h => h.x >= (currentItemHistory[currentItemHistory.length - 1].x - (days * 86400000))); }
    if(filteredData.length === 0) return;
    ...
    const options = {
        ...
        chart: { type: 'area', height: '100%', width: '100%', background: 'transparent', fontFamily: 'Inter, sans-serif', toolbar: { show: false }, animations: { enabled: true, easing: 'linear', dynamicAnimation: { speed: 1000 } } },
        colors: ['#2563eb'], stroke: { curve: 'smooth', width: 3 }, fill: { type: 'gradient', gradient: { shadeIntensity: 1, opacityFrom: 0.25, opacityTo: 0.0, stops: [0, 90, 100] } },
        markers: { size: 0, hover: { size: 6, colors: ['#ffffff'], strokeColors: '#2563eb', strokeWidth: 2 } }, dataLabels: { enabled: false }, 
        tooltip: {
            ...
            y: { formatter: (value) => `$${value.toFixed(2)}` }, style: { fontSize: '14px', fontFamily: 'Inter', fontWeight: 500 }
        },
        xaxis: {
            type: 'datetime', tickAmount: days >= 365 ? 5 : 6,
            labels: { style: { colors: '#334155', fontSize: '13px', fontFamily: 'Inter', fontWeight: 600 }, datetimeUTC: false, formatter: function(val) {
                if (!val) return ''; const date = new Date(val);
                if (days === 1) return date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false });
                if (days >= 365) return date.toLocaleDateString('en-US', { month: 'short', year: 'numeric' });
                return date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
            }}, axisBorder: { show: true, color: '#000000' }, axisTicks: { show: true, color: '#000000' }, crosshairs: { show: true, position: 'back', stroke: { color: '#64748b', width: 1, dashArray: 4 } }
        },
        yaxis: { opposite: false, min: minPrice - (minPrice * 0.002), max: maxPrice + (maxPrice * 0.002), labels: { style: { colors: '#334155', fontSize: '13px', fontFamily: 'Inter', fontWeight: 600 }, formatter: (value) => `$${value.toFixed(2)}` } },
        grid: { show: true, borderColor: '#000000', strokeDashArray: 4, position: 'back', xaxis: { lines: { show: true } }, yaxis: { lines: { show: true } }, padding: { top: 10, right: 20, bottom: 20, left: 10 } }
    };
    mainApexChart = new ApexCharts(container, options); mainApexChart.render();
}
...
async function fetchMarketData(isFirstLoad = false) {
    ...
    try {
        ...
        globalMarketData = data; container.innerHTML = ''; 

        data.forEach((item, index) => {
            const card = document.createElement('div'); card.className = 'card'; card.setAttribute('data-name', item.name); 
            if (isFirstLoad && index === 0) setTimeout(() => loadCustomApexChart(item), 100);
            const isPos = parseFloat(item.changePercent) >= 0; const sign = isPos ? '+' : '';
            card.innerHTML = `<div class="card-info"><div class="icon-box">${getEmojiIcon(item.name)}</div><div class="commodity-details"><h2>${item.name}</h2><div class="price">$${item.price.toFixed(2)}</div></div></div><div class="card-status"><div class="badge ${isPos ? 'positive' : 'negative'}">${sign}${item.changePercent}%</div></div>`;
            card.addEventListener('click', () => loadCustomApexChart(item)); container.appendChild(card);
        });
