...
let mainApexChart = null; // --- SAAT KAPSÜLÜ ---
function updateClock() {
    const now = new Date();
    const dateString = now.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
    const timeString = now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false });
...
function loadCustomApexChart(historyData) {
    ...
    const options = {
        series: [{ name: 'Price', data: historyData }],
        chart: {
            type: 'area', 
        ...
        yaxis: { show: false },
        grid: {
            show: true, borderColor: '#1a1d27', strokeDashArray: 5,
...
async function fetchMarketData(isFirstLoad = false) {
    ...
    try {
        // İŞTE SİHİRLİ DOKUNUŞ BURADA! Sonuna dinamik saat ekleyerek tarayıcıyı kandırıyoruz.
        const response = await fetch('data.json?v=' + new Date().getTime());
        const data = await response.json();
        
        // Eğer veri hala gelmediyse ekranı bozma
